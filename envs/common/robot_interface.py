...
class RobotInterface(object):
    ...
    def get_sensor_qposadr(self):
        """
        Returns the list of qpos indices of all actuated joints.
        """
        sensor_names = self.get_sensor_names()
        indices = [idx for idx, jnt in enumerate(sensor_names)]
        return [self.model.sensor_adr[i] for i in indices]
    ...
    def get_root_body_vel(self):
        qveladr = int(self.get_jnt_qveladr_by_name("root"))
    ...
    def get_sensordata(self, sensor_name):
        ...
        sensor_adr = self.model.sensor_adr[sensor_id.id]
        #print(f"sensor add = {sensor_adr}")
        data_dim = self.model.sensor_dim[sensor_id.id]
        #print(f"datadim = {data_dim}")
